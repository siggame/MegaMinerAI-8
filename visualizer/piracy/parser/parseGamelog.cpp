...
static bool parsePirateattack(Pirateattack& object, sexp_t* expression)
{
  sexp_t* sub;
  if ( !expression ) return false;
  object.type = PIRATEATTACK;
  sub = expression->list->next;
  if( !sub ) goto ERROR;
  object.attacker = atoi(sub->val);
  sub = sub->next;
  if( !sub ) goto ERROR;
  object.attackx = atoi(sub->val);
  sub = sub->next;
  if( !sub ) goto ERROR;
  object.attacky = atoi(sub->val);
  sub = sub->next;
  return true;


  ERROR:
  cerr << "Error in parsePirateattack.\n Parsing: " << *expression << endl;
  return false;
}
static bool parseShipattack(Shipattack& object, sexp_t* expression)
{
  sexp_t* sub;
  if ( !expression ) return false;
  object.type = SHIPATTACK;
  sub = expression->list->next;
  if( !sub ) goto ERROR;
  object.attacker = atoi(sub->val);
  sub = sub->next;
...
static bool parseSexp(Game& game, sexp_t* expression)
{
  ...
  else if(string(expression->val) == "animations")
  {
    ...
    while(expression->next)
    {
      ...
      if(string(ToLower( sub->val ) ) == "pirateattack")
      {
        Pirateattack* animation = new Pirateattack;
        if ( !parsePirateattack(*animation, expression) )
          return false;

        animations[ ((AnimOwner*)animation)->owner ].push_back( animation );
      }
      if(string(ToLower( sub->val ) ) == "shipattack")
      {
        Shipattack* animation = new Shipattack;
        if ( !parseShipattack(*animation, expression) )
          return false;

        animations[ ((AnimOwner*)animation)->owner ].push_back( animation );
      }
      if(string(ToLower( sub->val ) ) == "talk")
      {
